# Temu 售后客服话术

> 专业的 Temu 工厂价跨境电商售后客服话术模板，涵盖退换货处理、质量问题、包裹丢失、评价引导等售后服务场景。

## 使用说明

- 话术中的 `[变量]` 需要根据实际情况替换
- Temu 提供 90 天免费退货，要充分利用这一优势
- 主动提供积分补偿，提升客户满意度

---

## Temu 售后话术列表（共 15 条）

### 1. 退货政策说明

**适用场景：** 买家咨询退货流程

```
90-Day FREE Returns! 📦

How to Return:
Temu app → Orders → Return/Refund → Select reason → Print label

Benefits:
✓ Prepaid return label provided
✓ Refund within [Time] days after return received
✓ No restocking fees

Easy & hassle-free! ✨
```

---

### 2. 质量问题处理

**适用场景：** 买家反馈商品有质量问题

```
We're very sorry about the quality issue! 😔

Please send us photos/videos of the defect.

We'll immediately:
1) Full refund (keep the item) OR
2) Free replacement + return label

Your choice!

Plus [Credits] as apology.

We'll make it right! 💙
```

---

### 3. 商品不符处理

**适用场景：** 收到的商品与描述不符

```
Item not as described? We sincerely apologize! 🙏

Please send comparison photos.

Options:
A) Full refund + keep item + [Credits]
B) Free replacement

You decide!

Protected by Temu Purchase Protection!

Your satisfaction guaranteed! 💝
```

---

### 4. 退款处理

**适用场景：** 通知买家退款进度

```
Refund approved! 💰

Details:
• Amount: $[Amount]
• Method: [Original payment/Temu credits]
• Processing: 3-7 business days

You'll receive notification when completed.

Check "Orders" for status.

Plus [Bonus Credits] for the inconvenience!

Thank you! 🙏
```

---

### 5. 包裹丢失处理

**适用场景：** 买家反馈包裹丢失

```
Package lost or not delivered? We're so sorry! 😢

Don't worry - you're fully protected!

We'll:
1) Investigate with carrier
2) Issue immediate refund OR replacement

No waiting required!

Plus [Credits] as compensation.

We've got you covered! 🛡️
```

---

### 6. 评价奖励

**适用场景：** 引导买家留评

```
Love your purchase? 💕

Leave a review and get [Credits]! ⭐⭐⭐⭐⭐

Bonus:
• Share photos: +[Credits]
• Share videos: +[Credits]

Your feedback helps millions of shoppers!

Any issues? Contact us first - we'll resolve it + give you credits!

Thank you! 🙏
```

---

### 7. 换货处理

**适用场景：** 买家申请换货

```
Need an exchange? No problem! 🔄

Process:
1) Request exchange in app (Orders → Return/Refund)
2) Select new size/color
3) We ship replacement immediately
4) Return original with prepaid label

Or: Get refund and reorder.

90-day free returns! We've got you covered! 💙
```

---

### 8. 部分退款

**适用场景：** 协商部分退款解决问题

```
I understand your concern. 🤔

Since you'd like to keep the item, I can offer:

Option 1: [X]% partial refund ($[Amount]) + [Credits]
Option 2: Full refund with return (free return label)

My recommendation: Option 1 - you keep the item and get money back!

Which works for you? 💙
```

---

### 9. 差评挽回

**适用场景：** 买家留下差评后主动联系

```
Hi! I noticed your feedback and I'm truly sorry about your experience. 😔

I want to make this right!

I can offer:
• Full refund (keep the item)
• [Credits] bonus to your account
• Free replacement if you prefer

Please give me a chance to fix this.

Your satisfaction is my priority! 🙏
```

---

### 10. 客户回访

**适用场景：** 订单完成后回访客户

```
Hi! Just checking in! 👋

How's your [Product Name]? Is everything as expected?

We'd love to hear your feedback!

Happy with it?
Leave a review for [Credits]! ⭐

Any issues? Let me know - I'm here to help!

Thanks for choosing Temu! 💜
```

---

### 11. 物流超时安抚

**适用场景：** 订单超过预计送达时间但仍有物流轨迹

```
Thanks for your patience! I checked order [Order Number] and the package is still moving with [Carrier].

Current status: [Status]
Latest scan: [Latest Scan]
New estimated delivery: [New ETA]

If there is no new update by [Follow-up Date], we can help you request a refund or replacement through Temu Purchase Protection.

I'll keep monitoring this for you. 💙
```

---

### 12. 退货标签无法使用

**适用场景：** 买家反馈退货标签无法打印、无法扫码或网点不接收

```
Sorry the return label is not working! We'll help you fix it.

Please try:
1) Reopen the return page in the Temu app
2) Download the latest label
3) Show the barcode at [Drop Off Location]

If it still fails, send us a screenshot of the error. We will request a new label or guide you to another return method.

Your return window remains protected until [Return Deadline].
```

---

### 13. 重复退款申请说明

**适用场景：** 买家重复申请退款或担心退款未提交成功

```
I checked your case for order [Order Number]. Your refund request is already submitted.

Current refund status: [Refund Status]
Amount: $[Amount]
Expected processing time: [Time]

Submitting another request may slow down the review, so please keep this one active. If the status does not change by [Follow-up Date], we will escalate it for you.

Thank you for waiting. 🙏
```

---

### 14. 多件商品部分损坏

**适用场景：** 一个订单里多件商品，只有部分商品损坏或缺陷

```
I'm sorry part of your order arrived damaged.

Please send photos of:
1) The damaged item
2) The shipping package
3) The item label or SKU

For the affected item [Product Name], we can offer:
• Partial refund: $[Amount]
• Replacement if stock is available
• Full return if you prefer

The other items do not need to be returned if they are fine.
```

---

### 15. 优惠券补偿挽回

**适用场景：** 售后问题已解决后，用优惠券或积分鼓励复购

```
Thank you for giving us the chance to solve this.

As an apology for the inconvenience, we added:
• Coupon/Credits: [Coupon Amount]
• Valid until: [Date]
• Use on: [Eligible Items]

You can find it in your Temu account under coupons or credits. If anything still feels unresolved, message us before placing the next order and we'll help first.

We appreciate your patience. 💜
```

---

## 使用技巧

1. **强调 90 天退货**：这是 Temu 的核心优势，要反复提及
2. **积分补偿**：主动提供积分补偿，提升满意度
3. **快速响应**：售后问题要及时处理，避免纠纷升级
4. **灵活方案**：提供退款和换货两种选择
5. **评价激励**：用积分奖励引导买家留评

---

## 注意事项

- ✅ 可以用积分奖励评价
- ✅ 可以请求买家更新评价（解决问题后）
- ❌ 禁止好评返现
- ❌ 禁止威胁或骚扰买家

---

**更多话术模板：** [https://www.talkq.vip/templates/](https://www.talkq.vip/templates/)
